...
namespace RTS_Demo2
{
    public class GridController : MonoBehaviour
    {
        ...
        public Floor PickMobTarget
        {
            get
            {
                Floor[] inViewRangeFloors = PathFinder.GetTilesIntoACircle<Floor>(_map, CurrentMob.CurrentTile, CurrentMob.AttackRange);
                foreach (Character character in _characters)
                {
                    if (inViewRangeFloors.Contains(character.CurrentTile) && PathFinder.IsLineOfSightClear(_map, CurrentMob.CurrentTile, character.CurrentTile))
                    {
        ...
        private void Start()
        {
            ...
            foreach (Character charac in _characters)
            {
                charac.PathMap = PathFinder.GeneratePathMap(_map, charac.CurrentTile, charac.AllowDiagonals);
            }
            foreach (Character charac in _characters)
            {
                charac.PathMap = PathFinder.GeneratePathMap(_map, charac.CurrentTile, charac.AllowDiagonals);
        ...
        public void StartPreparingAttack()
        {
            ...
            Floor[] inViewRangeFloors = PathFinder.GetTilesIntoACircle<Floor>(_map, CurrentCharacter.CurrentTile, CurrentCharacter.AttackRange);
        ...
        public void DoPreparingAttack()
        {
            ...
            if (_hoveredFloor)
            {
                Floor[] inLineSightFloors = PathFinder.GetLineOfSight<Floor>(_map, CurrentCharacter.CurrentTile, _hoveredFloor, CurrentCharacter.AttackRange);
        ...
        public void StartIAPreparingAttack()
        {
            ...
            {
                _target = target;
                Floor[] inLineSightFloors = PathFinder.GetLineOfSight<Floor>(_map, CurrentMob.CurrentTile, target, CurrentMob.AttackRange);
                foreach (Floor floor in inLineSightFloors)
                {
                    floor.IsPath = true;
                }
