...
namespace KevinCastejon.Pathfinding2
{
    ...
    //public interface IPathTile : ITile
    //{
    //    public IPathTile Next { get; set; }
    //    public Vector2 NextDirection { get; set; }
    //    public int MovementSteps { get; set; }
    //    public float MovementCosts { get; set; }
    //}

    public class Node
    {

        internal Node(ITile tile, int x, int y)
        {
            _tile = tile;
            _x = x;
            _y = y;
        ...
        public ITile Tile { get => _tile; internal set => _tile = value; }
        public Node Next { get => _next; internal set => _next = value; }
    ...
    public class PathMap
    {
        ...
        private Dictionary<ITile, Node> _dico;
        private ITile _target;

        internal PathMap(Node[,] map, ITile target)
        {
            _target = target;
            _map = map;
            _flatMap = new Node[_map.GetLength(0) * _map.GetLength(1)];
            _dico = new Dictionary<ITile, Node>();
            int it = 0;
        ...
        public Node GetNodeFromTile(ITile tile)
        {
            return _dico[tile];
        }
        ...
        public T[] GetPathToTarget<T>(ITile tile) where T : ITile
        {
            ...
            return GetPathToTarget<T>(firstNode.X, firstNode.Y);
        }
        public T[] GetPathToTarget<T>(int tileX, int tileY) where T : ITile
        {
            Node node = _map[tileY, tileX];
        ...
        public T[] GetPathFromTarget<T>(int tileX, int tileY) where T : ITile
        {
            return GetPathToTarget<T>(tileX, tileY).Reverse().ToArray();
        }
        //public void InjectIntoTiles(IPathTile[,] grid)
        //{
        //    for (int i = 0; i < grid.GetLength(0); i++)
        //    {
        //        for (int j = 0; j < grid.GetLength(1); j++)
        //        {
        //            grid[i, j].Next = _map[i, j].Next;
        //            grid[i, j].NextDirection = _map[i, j].NextDirection;
        //            grid[i, j].MovementSteps = _map[i, j].MovementSteps;
        //            grid[i, j].MovementCosts = _map[i, j].MovementCosts;
        //        }
        //    }
        //}
    }
    public static class PathFinder
    {
        ...
        public static T[] GetTilesIntoACircle<T>(ITile[,] map, ITile start, int radius) where T : ITile
        {
            ...
            return GetTilesIntoACircle<T>(map, startX, startY, radius);
        }
        public static T[] GetTilesIntoACircle<T>(ITile[,] map, int startX, int startY, int radius) where T : ITile
        {
        ...
        public static T[] GetLineOfSight<T>(ITile[,] map, int startX, int startY, int stopX, int stopY, int maxDistance = 0) where T : ITile
        {
        ...
        //public static PathMap GeneratePathMapAndInjectIntoTiles(IPathTile[,] graph, IPathTile target, bool allowDiagonals = true, float diagonalWeightRatio = 1f)
        //{
        ...
        //        }
        //    }
        //    PriorityQueueUnityPort.PriorityQueue<Node, float> frontier = new();
        ...
        //                nei.Next = current;
        //                nei.NextDirection = new Vector2(nei.Next.X > nei.X ? 1 : (nei.Next.X < nei.X ? -1 : 0f), nei.Next.Y > nei.Y ? 1 : (nei.Next.Y < nei.Y ? -1 : 0f));
        //                nei.MovementSteps = current.MovementSteps + 1;
        //                nei.MovementCosts = newDistance;
        //                graph[nei.Y, nei.X].Next = (IPathTile)nei.Next;
        //                graph[nei.Y, nei.X].NextDirection = nei.NextDirection;
        //                graph[nei.Y, nei.X].MovementSteps = nei.MovementSteps;
        //                graph[nei.Y, nei.X].MovementCosts = nei.MovementCosts;
        //            }
        //        }
        //    }
        //    return new PathMap(map, target);
        //}
        public static PathMap GeneratePathMap(ITile[,] graph, ITile target, bool allowDiagonals = true, float diagonalWeightRatio = 1f)
        {
            int height = graph.GetLength(0);
            int width = graph.GetLength(1);
            Node targetNode = null;
            ...
            while (frontier.Count > 0)
            {
                ...
                foreach (Node nei in neibourgs)
                {
                    ...
                    if (nei.Next == null || newDistance < nei.MovementCosts)
                    {
                        frontier.Enqueue(nei, newDistance);
                        nei.Next = current;
                        nei.NextDirection = new Vector2(((Node)nei.Next).X > nei.X ? 1 : (((Node)nei.Next).X < nei.X ? -1 : 0f), ((Node)nei.Next).Y > nei.Y ? 1 : (((Node)nei.Next).Y < nei.Y ? -1 : 0f));
                        nei.MovementSteps = current.MovementSteps + 1;
                        nei.MovementCosts = newDistance;
                    }
                }
